...
class DatabaseServer implements IDatabaseServer {
    ...
    async getUser(id: number): Promise<IDatabaseUser | null> {
        const user: IDatabaseUser = await this.db.get('SELECT * FROM users WHERE id = $1 AND server = $2', [id, this.serverName]);
        return user;
    }

    async findByUserId(id: number | string): Promise<IDatabaseUser[]> {
        const users: IDatabaseUser[] = await this.db.all('SELECT * FROM users WHERE game_id = $1 AND server = $2 COLLATE NOCASE', [id, this.serverName]);
        return users;
    }
    ...
    async updateInfo(user: APIUser, mode: number): Promise<void> {
        const dbUser = await this.db.get('SELECT * FROM stats WHERE id = $1 AND mode = $2 AND server = $3 LIMIT 1', [user.id, mode, this.serverName]);
        if (!dbUser.id) {
    ...
    async getUserStats(id: number, mode: number): Promise<IDatabaseUserStats> {
        ...
        const stats: IDatabaseUserStats = await this.db.get('SELECT * FROM stats WHERE id = $1 AND mode = $2 AND server = $3', [u.game_id, mode, this.serverName]);
        return stats;
    }
}
...
class DatabaseUsersToChat {
    ...
    async isUserInChat(userId: number, chatId: number): Promise<boolean> {
        const user = await this.db.get('SELECT * FROM users_to_chat WHERE user_id = $1 AND chat_id = $2', [userId, chatId]);
        return !!user.user_id;
    }
}
