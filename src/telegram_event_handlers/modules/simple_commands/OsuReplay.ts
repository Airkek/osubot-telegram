    ...
    constructor(module: SimpleCommandsModule) {
        super(["osu_replay"], module, async (ctx) => {
            ...
            if (canRender && settings.experimental_renderer) {
                renderAdditional +=
                    "\n⚠️Используется экспериментальный рендерер. Некоторые функции могут быть недоступны или работать неправильно, а скорость рендера может быть низкой из-за отсутствия видеокарты на сервере у бота.\n\nПросьба сообщать о найденных ошибках в комментарии к посту - https://t.me/osubotupdates/34";
            }
