    ...
    constructor(module: Module) {
        super(
            ...
            async (self) => {
                ...
                const map =
                    recent.beatmap ?? (await self.module.api.getBeatmap(recent.beatmapId, recent.mode, recent.mods));

                const cover = map.coverUrl
                    ? await self.module.bot.database.covers.getPhotoDoc(map.coverUrl)
                    : await self.module.bot.database.covers.getCover(map.id.set);
                ...
                const keyboard =
                    self.module.api.getScore !== undefined
                        ...
                        : undefined;
                await self.reply(`${self.module.bot.templates.ScoreFull(recent, map, calc, self.module.link)}`, {
                    attachment: cover,
                    keyboard,
                });
