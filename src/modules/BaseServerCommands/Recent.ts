...
export default class AbstractRecent extends ServerCommand {
    constructor(module: Module) {
        super(['recent', 'r', 'rp', 'к', 'кз', 'кусуте'], module, async (self) => {
            ...
            const map = recent.beatmap ?? await self.module.api.getBeatmap(recent.beatmapId, recent.mode, recent.mods);
            
            const cover = map.coverUrl ? await self.module.bot.database.covers.getPhotoDoc(map.coverUrl) : await self.module.bot.database.covers.getCover(map.id.set);
            ...
            await self.reply(`${self.module.bot.templates.RecentScore(recent, map, calc, self.module.link)}`, {
                attachment: cover,
                keyboard
            });
