    ...
    constructor(module: Module) {
        super(['compare', 'c', 'с', 'сщьзфку'], module, async (self) => {
            ...
            const score = self.user.username 
                ...
                : await self.module.api.getScoreByUid(self.user.id || self.user.dbUser.game_id, chat.map.id.map, mode, self.args.mods.length == 0 ? undefined : new Mods(self.args.mods).sum());
            ...
            await self.reply(`${self.module.bot.templates.Compare(score, map, calc)}`, {
                attachment: cover
            });
        }, true);
    }
