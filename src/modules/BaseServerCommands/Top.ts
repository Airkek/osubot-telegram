    ...
    constructor(module: Module, ignoreDbUpdate: boolean = false) {
        super(['top', 't', 'е', 'ещз'], module, async (self) => {
            ...
            if (self.args.apx) {
                ...
                const map = nearest.beatmap ?? await self.module.api.getBeatmap(nearest.beatmapId, mode, nearest.mods);
                ...
                await self.reply(`У игрока ${user.nickname} ${amount ? amount : 'нет'}${amount == 100 ? '+' : ''} ${Util.scoreNum(amount)} выше ${self.args.more}pp`);
            } else if (self.args.place) {
                const score = (await self.module.api.getUserTopById(user.id, mode, self.args.place))[self.args.place - 1];
                ...
                const keyboard = self.module.api.getScore !== undefined ? Util.createKeyboard([
                    [{
                ...
                await self.reply(`${self.module.bot.templates.TopSingle(score, map, user, self.args.place, calc, self.module.link)}`, {
                    attachment: cover,
                    keyboard
                });
                self.module.bot.maps.setMap(self.ctx.peerId, map);
                ...
                top = top.splice(0, 3);
                const maps = await Promise.all(top.map((s) => s.beatmap ? Promise.resolve(s.beatmap) : self.module.api.getBeatmap(s.beatmapId, mode, s.mods)));
                const str = maps.map((map, i) => {
