    ...
    constructor(module: Module, ignoreDbUpdate: boolean = false) {
        super(
            ...
            async (self) => {
                ...
                if (self.args.apx) {
                    let top = await self.module.api.getUserTopById(
                        user.id,
                        mode,
                        100
                    );
                    if (self.args.mods) {
                        ...
                        top = top.filter(
                            (score) => score.mods.sum() == mods.sum()
                        );
                        if (!top[0]) {
                            await self.reply(
                                "Не найдено топ скоров с указанной комбинацией модов!"
                            );
                            return;
                        }
                    }
                    ...
                    const score = (
                        await self.module.api.getUserTopById(
                            user.id,
                            mode,
                            self.args.place
                        )
                    ...
                    const str = maps
                        .map((map, i) => {
                            ...
                            return self.module.bot.templates.TopScore(
                                top[i],
                                map,
                                i + 1,
                                calc,
