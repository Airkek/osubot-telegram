    ...
    constructor(module: Module, ignoreDbUpdate: boolean = false) {
        super(["top", "t", "е", "ещз"], module, async (self) => {
            ...
            if(self.args.apx) {
                ...
                let map = await self.module.api.getBeatmap(nearest.beatmapId, mode, nearest.mods.diff());
                ...
                let map = await self.module.api.getBeatmap(score.beatmapId, mode, score.mods.diff());
                let cover = await self.module.bot.database.covers.getCover(map.id.set);
                ...
                top = top.splice(0, 3);
                let maps = await Promise.all(top.map(s => self.module.api.getBeatmap(s.beatmapId, mode, s.mods.diff())));
