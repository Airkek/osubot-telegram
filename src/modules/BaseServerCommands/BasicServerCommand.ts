...
const createServerCommandRunner = (func: (self: CommandContext) => Promise<void>, needUserParse: boolean): (ctx: UnifiedMessageContext, self: Command, args: ICommandArgs) => Promise<void> => {
        ...
        if (needUserParse) {
            if (context.ctx.hasReplyMessage) {
                ...
                if (!context.user.dbUser.nickname && !args.nickname[0]) {
                    await context.reply(`У этого пользователя не указан ник!\nПривяжите через ${context.module.prefix[0]} nick <ник>`);
                    return;
                }
