...
export class PostgresIdentityRepository implements IdentityRepository {
    ...
    async findUser(externalId: ExternalId): Promise<PlatformAccountIdentity | null> {
        return this.findUserWith(this.db, externalId);
    }

    async resolveUser(externalId: ExternalId): Promise<PlatformAccountIdentity> {
    ...
    async getUser(accountId: number): Promise<PlatformAccountIdentity | null> {
    ...
    async findChat(externalId: ExternalId): Promise<PlatformChatIdentity | null> {
    ...
    async resolveChat(externalId: ExternalId): Promise<PlatformChatIdentity> {
        const normalizedId = normalizeExternalId(externalId);
        const row = await this.db.get<PlatformChatRow>(
            `WITH inserted AS (
    ...
    async getChat(chatId: number): Promise<PlatformChatIdentity | null> {
        const row = await this.db.get<PlatformChatRow>(
            `SELECT id AS chat_id, platform, external_id
             FROM platform_chats
             WHERE id = $1 AND platform = $2`,
    ...
    async getUserAccounts(userId: number): Promise<PlatformAccountIdentity[]> {
    ...
    private async findUserWith(db: SqlExecutor, externalId: ExternalId): Promise<PlatformAccountIdentity | null> {
    ...
    private mapUser(row: PlatformAccountRow): PlatformAccountIdentity {
        return {
    ...
    private mapChat(row: PlatformChatRow): PlatformChatIdentity {
        return {
