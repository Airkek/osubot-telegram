...
import { UserSettings } from "../src/core/Settings";
import { UserSettingsModel } from "../src/data/Models/Settings/UserSettingsModel";
import { SqlExecutionResult, SqlExecutor, SqlParameters } from "../src/data/SqlExecutor";

class SettingsSqlFake implements SqlExecutor {
    readonly writes: Array<{ statement: string; parameters?: SqlParameters }> = [];

    constructor(private readonly rows: Map<number, UserSettings>) {}
    ...
    async run(statement: string, parameters?: SqlParameters): Promise<SqlExecutionResult> {
