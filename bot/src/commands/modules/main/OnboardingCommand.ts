...
import { ONBOARDING_VERSION } from "core/storage/OnboardingVersion";
...
interface StepInfo {
    ...
    postprocess?(userId: number, onboardingModel: IOnboardingRepository): Promise<void>;
}
...
function previousStepButton(userId: number, step: OnboardingSteps, l: ILocalizer): IKeyboardButton {
    return {
...
function buildFlowButtons(userId: number, currentStep: OnboardingSteps, l: ILocalizer): IKeyboard {
...
function stepLang(userId: number, l: ILocalizer): StepData {
...
function buildBoolButton(userId: number, step: OnboardingStepsYesNo, value: boolean, l: ILocalizer) {
...
function stepBoolean(userId: number, currentStep: OnboardingStepsYesNo, text: string, l: ILocalizer): StepData {
    return {
        step: currentStep,
        text: text,
...
function stepAutoRender(userId: number, l: ILocalizer): StepData {
...
function stepExperimentalRender(userId: number, l: ILocalizer): StepData {
...
function stepFinal(userId: number, l: ILocalizer): StepData {
    return {
        step: "final",
...
async function finalPostProcess(userId: number, onboardingModel: IOnboardingRepository) {
    await onboardingModel.userOnboarded(userId, ONBOARDING_VERSION);
}

export class OnboardingCommand extends Command {
    onboardingModel: IOnboardingRepository;
    ...
    async applyStepSet(step: OnboardingStepsWithSetters, value: string, ctx: IMessageContext) {
        ...
        await ctx.reactivateLocalizer();
    }

    async applyAutoRenderSet(step: OnboardingStepsYesNo, value: BoolParams, ctx: IMessageContext) {
    ...
    async applyExperimentalRenderSet(step: OnboardingStepsYesNo, value: BoolParams, ctx: IMessageContext) {
        const settings = await ctx.userSettings();
        settings.experimental_renderer = extractBoolFromParam(value);
        await ctx.updateUserSettings(settings);
    }

    async applyLanguageSet(step: LanguageStep, value: LangParams, ctx: IMessageContext) {
        const settings = await ctx.userSettings();
        switch (value) {
            case "ru":
