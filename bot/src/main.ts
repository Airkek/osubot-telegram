...
import { TelegramBotAdapter } from "platforms/telegram/TelegramBotAdapter";
import { ITelegramBotConfig } from "platforms/telegram/ITelegramBotConfig";
import { VKBotAdapter } from "platforms/vk/VKBotAdapter";
...
import { PlatformStorage } from "core/PlatformStorage";
import { ApplicationHttpServer } from "application/ApplicationHttpServer";
import { runtimePaths } from "application/RuntimePaths";
import { FluentLocalizer } from "localization/FluentLocalizer";
import { Platform } from "core/Platform";
import { SUPPORTED_PLATFORMS } from "core/Platform";
import { PostgresStorage } from "infrastructure/database/PostgresStorage";
...
let bots: IPlatformAdapter[] = [];
let runtime: ApplicationRuntime;
const httpServer = new ApplicationHttpServer();
const localizer = new FluentLocalizer(runtimePaths.locales);
let stopping = false;
...
function createPlatformAdapter(platform: Platform): IPlatformAdapter {
    switch (platform) {
        case "vk": {
            const config: IVKBotConfig = {
        ...
        case "telegram": {
            const config: ITelegramBotConfig = {
...
async function main(): Promise<void> {
    ...
    httpServer.start();
    const started: IPlatformAdapter[] = [];
    try {
