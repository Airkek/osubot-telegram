...
import { VideoSendFailedError } from "core/errors/VideoSendFailedError";
...
import { uploadMessagePhoto, uploadMessageVideo } from "platforms/vk/Upload";
...
export class VKMessageContext extends BaseMessageContext {
    ...
    private async prepareMessage(text: string, options?: ISendOptions): Promise<PreparedMessage> {
        if (options?.photo) {
        ...
        if (options?.video) {
            if (!this.userVk) {
                ...
                throw new VideoSendFailedError("VK_USER_TOKEN is not configured");
            }
            try {
                ...
                throw new VideoSendFailedError("Failed to upload video to VK");
            }
        }
