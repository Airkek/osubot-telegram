...
export { ControllableFeature } from "core/storage/ControllableFeature";
...
export class FeatureControlModel {
    ...
    async isFeatureEnabled(feature: ControllableFeature): Promise<boolean> {
        ...
        const featureStatus = await this.db.get<IFeatureStatus>("SELECT * FROM feature_control WHERE feature = $1", [
            feature,
        ]);
    ...
    async listFeatures(): Promise<IFeatureStatus[]> {
        return await this.db.all<IFeatureStatus>("SELECT * FROM feature_control");
    }
