...
export class StatisticsModel {
    ...
    public async logRenderStart(ctx: IEventContext, mode: number, isExperimental: boolean) {
        return await this.logRenderEvent("render_start", ctx, mode, isExperimental);
    }

    public async logRenderSuccess(ctx: IEventContext, mode: number, isExperimental: boolean) {
        return await this.logRenderEvent("render_success", ctx, mode, isExperimental);
    }

    public async logRenderFailed(ctx: IEventContext, mode: number, errorMessage: string, isExperimental: boolean) {
        return await this.logRenderEvent("render_failed", ctx, mode, isExperimental, errorMessage);
    }

    public async logMessage(ctx: IEventContext) {
        return await this.logRawEvent("new_message", ctx);
    }

    public async logCommand(command: ICommandEvent, ctx: IEventContext) {
        await this.db.run(
    ...
    public async logStartup(me: IBotIdentity) {
        await this.db.run(
    ...
    private async logRawEvent(type: RawEvents, ctx: IEventContext) {
        try {
            await this.db.run(
