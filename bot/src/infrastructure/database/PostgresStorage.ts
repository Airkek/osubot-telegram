import { ISqlConnection } from "infrastructure/database/ISqlConnection";
import { ISqlExecutor } from "infrastructure/database/ISqlExecutor";
import { Pool } from "pg";
import { createHash } from "node:crypto";
import { applyMigrations } from "infrastructure/database/Migrations";
import { FeatureControlModel } from "infrastructure/database/models/FeatureControlModel";
import { NotificationsModel } from "infrastructure/database/models/NotificationsModel";
import { ChatSettingsModel } from "infrastructure/database/models/settings/ChatSettingsModel";
import { UserSettingsModel } from "infrastructure/database/models/settings/UserSettingsModel";
import { OsuBeatmapCacheModel } from "infrastructure/database/models/OsuBeatmapCacheModel";
import { StatisticsModel } from "infrastructure/database/models/StatisticsModel";
import { ChatMembersModel } from "infrastructure/database/models/ChatMembersModel";
import { OnboardingModel } from "infrastructure/database/models/OnboardingModel";
import { UserInfoModel } from "infrastructure/database/models/UserInfoModel";
...
import { MediaReferenceModel } from "infrastructure/database/models/MediaReferenceModel";
import { MaintenanceModel } from "infrastructure/database/models/MaintenanceModel";
import { Platform } from "core/Platform";
import { PostgresIdentityRepository } from "infrastructure/database/repositories/PostgresIdentityRepository";
import { PostgresGameUserRepository } from "infrastructure/database/repositories/PostgresGameUserRepository";
import { PostgresIdentityLinkRepository } from "infrastructure/database/repositories/PostgresIdentityLinkRepository";
...
class PostgresErrorStore {
    ...
    async addError(ctx: IErrorContext, error: unknown): Promise<string> {
    ...
    async getError(code: string): Promise<IStoredError | null> {
        return await this.db.get<IStoredError>("SELECT * FROM errors WHERE code = $1", [code]);
    }
...
export class PostgresStorage implements IApplicationStorage {
    ...
    constructor(
        private readonly db: ISqlConnection,
