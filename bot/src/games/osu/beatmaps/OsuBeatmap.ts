...
import { calculateBeatmap } from "games/osu/performance/OfficialCalculator";
import { IBeatmapMetadata } from "core/storage/IBeatmapMetadata";

class OsuBeatmapStats implements IBeatmapStats {
    readonly ar: number;
    readonly cs: number;
    readonly od: number;
    readonly hp: number;
...
export class OsuBeatmap implements IBeatmap {
    ...
    constructor(apiBeatmap?: IBeatmapData, dbBeatmap?: IBeatmapMetadata) {
        if (apiBeatmap) {
